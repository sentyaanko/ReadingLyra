...
function Update-SaNavisYml{
	...
	$RootMdFiles = Get-ChildItem *.md -Exclude index.md | ForEach-Object {$Heading=(Get-Content $_.FullName -TotalCount 1 -Encoding utf8) -replace '^# (.+) \<\!-- omit in toc --\>', '$1';$RelativePath=((Resolve-Path $_ -Relative) -replace '\\','/') -replace '/(.+)\.md', '/$1';@{Heading=$Heading; RelativePath=$RelativePath}}
