...
* 新たなプラグイン ShooterExplorer と ShooterTest にプロトタイプとテスト用のコンテンツを追加しました。
	...
	> * `ShooterTests`
	> 	* `AFunctionalTest` を使用したテスト用のファイルが置かれています。
	> 	* 詳しくは以下などを参照してください。
...
* アビリティが Health にネガティブモディファイアを適用する代わりに、 Healing と Damage のアトリビュートを適切に使用するように変更しました。
	...
	> 		* Health を減らす際、 Damage を介するように変更されました。
	> * `ULyraDamageExecution`
	> 	* ターゲットの Health アトリビュートのキャプチャが削除されました。
	> 		* これは元々 Health 以上のダメージを与えない処理のためでしたが、 `ULyraHealthSet` 側で行うように変更されました。
	> * `ULyraHealExecution`
	> 	* モディファイアのアトリビュートの指定が Health から Healing に変更されました。
	> 	* 直接 Health の変更していたのを Healing を介するように変更されました。
