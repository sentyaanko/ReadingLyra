...
- [2. Animation Layer Interface](#2-animation-layer-interface)
	- [2.1 用途](#21-用途)
...
* [Unreal Engine 5.1 Documentation > サンプルとチュートリアル > サンプル ゲーム プロジェクト > Lyra サンプル ゲーム > Lyra のアニメーション]
	> Lyra の Animation Blueprint で使われている機能がまとめられています。
...
* [技術ブログ > Lyra アニメーションを UE5 ゲームに適応する方法について]
	> Lyra の Animation Blueprint の改造の手順が解説されています。  
	> アニメーションを置き換えたり、移動方向にアクターが向くような改造を通じて Animation Blueprint で行っていることやパラメータの調整方法など色々なことが学べます。
...
[TurnYawAnimModifier] は Lyra で実装されている Animation Modifier です。  
TurnInPlace では 90 度振り向く Animation Sequence を利用しますが、この Animation Sequence に Animation Modifier を設定しています。
この Animation Sequence ではルートモーションで Yaw が変化する様に作られています。  
Animation Modifier では回転量及び回転量が最終フレームと同値になるタイミングを示す Animation Curve を自動生成します。
Animation Curve の内容は以下のようになります。
...
| 						| 休憩用の Animation Sequence を `Sequence Player` で再生します。						|
...
* `IdleBreak` -> `Idle`
	* Animation Sequence の再生が終わるか、何らかの操作（詳細は省略）をすると移行します。
* `*` -> `TurnInPlaceRotation`
...
* `TurnInPlaceRecovery` -> `Idle`
	* Animation Sequence の再生が終わると移行します。
...
* `StanceTransition` -> `Idle`
	* Animation Sequence の再生が終わるか、立ち・しゃがみ状態の変化を検知すると移行します。
...
1. 基本となる Animation Sequence の決定
...
* Animation Sequence の設定
	* 使用する Animation Sequence の例として、 `MM_Pistol_Jog_Fwd_Strat` の場合でを見てみましょう。  
	* このアセットのAnimation Curve `Distance` の 0 フレーム付近の値の変化は以下のようになっています。
...
0.15 秒までの間、各フレームで Animation Sequence のどこまで進んでいるかを考えます。

仮に、極端に遅い移動速度を 0.3 とします。  
移動速度 0.3 を 30 fps でのフレーム毎の移動量に換算すると 0.001 となるので、毎フレーム 0.001 ずつ増えていきます。

| 時間(フレーム)	| 起点からの移動量	| Animation Sequence の位置(フレーム)	| 前フレームからの再生レート	| アニメーション上の起点からの移動量	|
|----				|----				|----									|----							|----									|
| 0					| 0.000				| 0.750									|								| 0.000									|
...
0.2 秒までの間、各フレームで Animation Sequence のどこまで進んでいるかを考えます。
...
その後の Animation Sequence 側の移動量は増えていくため、その差はどんどん広がります。  
ですが 0.2 秒の段階で、 **ストライド ワーピング** のパラメータ `Alpha` が `1.0` となり、以降はそちらに頼ることになります。


#### 3.2.1.3 移動速度が極端に速い場合

アニメーション開始直後、各フレームで Animation Sequence のどこまで進んでいるかを考えます。
...
> * ブレンドのウェイトは Animation Curve `DisableLeftHandPoseOverride` を利用することで調整可能になっています。
> 	* ですがこの Animation Curve を持つアニメーションは存在しません。
