...
* すべてアセットがあるわけではなく、用意されていないものは別の性別、武器種のものが設定されています。
	* 例：Shotgun はその殆どが Rifle を流用しています。 Crouch は MF のアセットしかありません。
...
* アセットの有無の表
	...
	* 例: ```[MF|MM]_[Pistol|Rifle|Shotgun|Unarmed]_Idle_ADS```
		* Rifle 列が `-F` 、 Shotgun が `M-` となっており、これは以下の2ファイルが存在することを意味しています。
			* ```MF_Rifle_Idle_ADS```
			* ```MM_Shotgun_Idle_ADS```
...
また、 `BS_MM_Rifle_Jog_Leans` は基底クラス [ABP_Mannequin_Base] で直接指定されており、武器種などのバリエーションもありません。
...
## 5.1. 各 Animation Montage と Pose Asset の参照元
...
Animation Montage と Pose Asset 、どちらも (男女 2) x (関節 7) x (左右 2) の 28 のバリエーションを持っています。
